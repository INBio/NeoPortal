...
<!-- Attributes -->
<%@ attribute name="taxon" description="The taxon description of the element" required="true" type="org.inbio.neoportal.core.entity.Taxon" %>
	...
	<div id="taxonomy">
		<h3><fmt:message key="taxonDes.taxonomy"/></h3>
   		<ul>
   			...
   			<c:if test="${not empty taxon.phylum }">
   			<li><fmt:message key="taxon.phylum"/>: <span>${taxon.phylum}</span></li>
   			</c:if>
   			<c:if test="${not empty taxon.class_ }">
   			<li><fmt:message key="taxon.class_"/>: <span>${taxon.class_}</span></li>
   			</c:if>
